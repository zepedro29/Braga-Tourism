...
http.open('get', 'alojamentos.json', true);
let alojamentos;
http.send();
...
http.onload = function(){
	...
	if(this.readyState == 4 && this.status == 200){
		 ...
		 alojamentos = JSON.parse(this.responseText);

		let output = "";

		for(let item of alojamentos){
			output += `
      <div class="row pt-5">
  ...
  <div class="col-12 col-md-6 pt-3 text-left" style="left: -50px">
    ...
    <div class="row text-left">
      <p style="font-size: 15px;"> <i class="bi bi-telephone-fill"></i>&emsp;<strong>${item.telemovel}</strong></p>
		...
		document.querySelector(".alojamentos").innerHTML = output;
	}
} 

const checkboxes = document.querySelectorAll('.categoria');
checkboxes.forEach(checkbox => checkbox.addEventListener('change', filtraralojamentos));

function filtraralojamentos() {
  ...
  const alojamentosFiltrados = alojamentos.filter(alojamento => {
    if (categoriasSelecionadas.length === 0) {
      ...
      return categoriasSelecionadas.includes(alojamento.categoria);
    }
  });

  exibiralojamentos(alojamentosFiltrados);
}

function exibiralojamentos(alojamentos) {
  let output = "";
